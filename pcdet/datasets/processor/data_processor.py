...
class DataProcessor(object):
    ...
    def shuffle_points(self, data_dict=None, config=None):
        ...
        if config.SHUFFLE_ENABLED[self.mode]:
            ...
            shuffle_idx = np.random.permutation(lidar_points.shape[0])
            shuffle_idx = np.random.permutation(radar_points.shape[0])
            # points = points[shuffle_idx]
            lidar_points = lidar_points[shuffle_idx]
            radar_points = radar_points[shuffle_idx]
