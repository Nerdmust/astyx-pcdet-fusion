        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                if count_inside_pts:
                    ...
                    # rotate
                    radar_points_rotate = np.dot(radar_points[:, :3], calib['T_from_lidar_to_radar'][:3, :3])
                    # translation
                    radar_points_translation_x = radar_points_rotate[:, :1] - calib['T_from_lidar_to_radar'][:3, 3:4][0]
                    radar_points_translation_y = radar_points_rotate[:, 1:2] - calib['T_from_lidar_to_radar'][:3, 3:4][1]
                    radar_points_translation_z = radar_points_rotate[:, 2:3] - calib['T_from_lidar_to_radar'][:3, 3:4][2]
                    # concatenate
                    radar_points = np.concatenate([radar_points_translation_x, radar_points_translation_y, radar_points_translation_z, radar_points[:, 3:]], axis=1)
